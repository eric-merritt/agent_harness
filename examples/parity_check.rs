...
fn main() {
	...
	let (t_pure, m_pure) =
		agent_harness::models::dedupe::tensor::DedupCountTensor::compress_from_gpu(
			&out.prefix_ints,
			&out.tails,
			&out.signs,
			prefix_digits,
	...
	let (t_avx, m_avx) =
		agent_harness::models::dedupe::tensor::DedupCountTensor::compress_gpu_w_avx512(
			&weights,
			&out.prefix_bits,
			&out.tails,
			&out.signs,
	...
	println!(
		"unique_tails equal: {}",
		t_pure.unique_tails == t_avx.unique_tails
	);
	println!(
		"prefix_counts equal: {}",
		t_pure.prefix_counts == t_avx.prefix_counts
	);
	println!("count: {} vs {}", t_pure.count, t_avx.count);
	println!(
		"avg_precision_lost: {} vs {}",
		t_pure.avg_precision_lost, t_avx.avg_precision_lost
	);

	// Compare sandbags element-wise
	let mut tail_diff = 0;
	for i in 0..n {
		if m_pure.tail_idx[i] != m_avx.tail_idx[i] {
			tail_diff += 1;
		}
		if m_pure.prefix_idx[i] != m_avx.prefix_idx[i] {
			tail_diff += 1;
		}
	}
	let mut sign_diff = 0;
	for i in 0..((n + 7) / 8) {
		if m_pure.sign_bits[i] != m_avx.sign_bits[i] {
			sign_diff += 1;
		}
	}
	println!(
		"tail_idx/prefix_idx diffs: {}  sign_bits byte diffs: {}",
		tail_diff, sign_diff
	);
