...
pub enum LoRADtype {
    F16, BF16, F32, I8, I16, I32, I64, U8, Bool,
}
...
impl ModelAdapter for LoRAAdapter {
    fn model_name(&self) -> &str { &self.base_model }

    fn tensor_count(&self) -> usize { self.tensors.len() }
    ...
    fn raw_data(&self, tensor_name: &str) -> Option<&[u8]> {
        self.tensors.get(tensor_name).map(|info| {
