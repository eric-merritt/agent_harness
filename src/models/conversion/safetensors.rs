...
use super::super::compression::tensor::CompressedTensor;
...
pub fn convert_safetensors(
	...
	log::info!(
		"Converting safetensors: {} ({} tensors)",
		model_name,
		tensor_count
	);
	...
	for (i, (name, info)) in sorted.into_iter().enumerate() {
		...
		let out = CompressedTensor::compress_job(&weights, pd, tr);
	...
	let overall_ratio = if total_core > 0 {
		total_original as f32 / total_core as f32
	} else {
		1.0
	};
	let stats = ConversionStats {
		...
		total_core_bytes: total_core,
		total_sandbag_bytes: total_meta,
...
pub fn convert_safetensors_parallel(
		...
		.map(|_| {
			...
			thread::spawn(move || {
				loop {
					...
					let out = CompressedTensor::compress_job(&job.weights, pd, tr);
					...
					let _ = result_tx.send(CompressResult {
						...
						stats: TensorStats {
							...
							original_bytes: job.element_count * 4,
							core_bytes: out.core.len(),
							sandbag_bytes: out.sandbag.len(),
							core_ratio: (job.element_count * 4) as f32
								/ out.core.len().max(1) as f32,
							prefix_count: out.prefix_count,
							unique_tail_count: out.unique_tail_count,
							shared_weights: out.shared_weights,
							mean_precision_lost: out.mean_precision_lost,
	...
	let reader_handles: Vec<_> = (0..2)
		.filter_map(|rid| {
			...
			let my_tensors: Vec<_> = global_tensors
				.iter()
				.filter(|(_, si, _, _, _, _, _, _)| *si >= rs && *si < re)
				.cloned()
				.collect();
			...
			Some(thread::spawn(move || {
				...
				for t in my_tensors {
					by_shard.entry(t.1).or_default().push(t);
				}
				for (si, tensors) in by_shard.iter() {
					...
					for (gi, _, name, dtype, shape, doff, bsize, dstart) in tensors {
						...
						let raw = &data[(dstart + *doff as usize)
							..(dstart + *doff as usize + *bsize as usize)];
						let weights = dtype.dequantize_to_f32(raw, ne);
						let _ = job_tx.send(CompressJob {
							global_idx: *gi,
							name: name.clone(),
							shape: shape.clone(),
							element_count: ne,
	...
	let overall = if t_core > 0 {
		t_orig as f32 / t_core as f32
	} else {
		1.0
	};
	let final_model_name = shard_paths[0]
		.file_stem()
		.and_then(|s| s.to_str())
		.unwrap_or("unknown")
		.to_string();
