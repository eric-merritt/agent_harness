// Core dictionary binary format: serialize/deserialize CompressedTensor.
//
...
use crate::models::compression::tensor::CompressedTensor;
use crate::models::formats::sandbag::{UniqueTail};
use crate::models::compression::types::DataFlag;
/// Serialize the core dictionary using the bidirectional layout.
pub fn serialize_core(tensor: &CompressedTensor) -> Vec<u8> {
...
pub fn deserialize_core(data: &[u8]) -> Option<CompressedTensor> {
	let mut pos = 0;
	deserialize_core_at(data, &mut pos)
}

/// Deserialize a CompressedTensor starting at `pos`, advancing pos past the data.
pub fn deserialize_core_at(data: &[u8], pos: &mut usize) -> Option<CompressedTensor> {
	if data.len() < *pos + 24 {
		return None;
	}
	...
	*pos += 24;
	...
	if *pos < data.len() && data[*pos] == DataFlag::GapFlag as u8 {
		*pos += 1;
	}
	...
	if data.len() < *pos + total_rear_bytes {
		return None;
	}
	...
	if data[tail_flag_pos] != DataFlag::TailFlag as u8 {
		return None;
	}
	if data[count_flag_pos] != DataFlag::CountFlag as u8 {
		return None;
	}
	...
	for i in 0..prefix_count {
		...
		prefix_counts[prefix_count - 1 - i] =
			u32::from_le_bytes(data[chunk_offset..chunk_offset + 4].try_into().ok()?);
	}

	*pos = count_flag_pos + 1;

	Some(CompressedTensor {
		count,
		prefixes,
		unique_tails,
		prefix_counts,
...
pub fn deserialize_core_chunks(data: &[u8]) -> Vec<CompressedTensor> {
	if data.len() < 4 {
		return Vec::new();
	}
