...
pub fn convert_gguf(
	...
	let loader_handles: Vec<_> = (0..gpu_loaders)
		.map(|_| {
	...
	let worker_handles: Vec<_> = (0..cpu_workers)
		.map(|_| {
			let job_rx = Arc::clone(&job_rx);
			let result_tx = result_tx.clone();
			thread::spawn(move || {
	...
	let reader = thread::spawn(move || -> Result<(), String> {
		...
		for (tpidx, (tidx, n_elems, dtype, nchunks)) in tensor_plan2.iter().enumerate() {
			...
			let (block_elems, block_bytes) =
				crate::models::formats::gguf::quant_block_info(*dtype).unwrap_or((1, 4));
			let blocks_per_chunk = (CHUNK_SIZE + block_elems - 1) / block_elems;
			let chunk_bytes = blocks_per_chunk * block_bytes;
			let total_bytes = info.byte_size() as usize;
			...
			for c in 0..*nchunks {
				...
				let raw_chunk = gguf2
					.read_tensor_range(&mut file, *tidx, byte_offset, read_len)
	...
	while expected_chunk < total_chunks {
		...
		buffer.insert(
			result.global_idx,
			(result.core, result.sandbag, result.stats),
		);
