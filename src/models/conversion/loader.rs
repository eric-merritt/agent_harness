...
impl ModelLoader {
	...
	pub fn get_tensor_slices(
		&self,
		name: &str,
	) -> Result<(&[u8], &[u8], TensorStats), Box<dyn std::error::Error>> {
		let idx = *self
			.tensor_index
			.get(name)
		...
		Ok((
			&self.weights_mmap[w_start..w_end],
			&self.sandbag_mmap[s_start..s_end],
			stats,
		))
	}
	...
	fn decompress_tensor_impl(
		...
		for _ in 0..n_chunks {
			...
			chunks.push(ChunkInfo {
				core_pos: chunk_core_pos,
				sand_pos: chunk_sand_pos,
				element_count: tensor.count,
				remap,
			});
		}
		...
		if parallel_chunks && chunks_data.len() > 1 {
			let work_queue = Arc::new(Mutex::new(
				(0..chunks_data.len()).collect::<VecDeque<usize>>(),
			));
				...
				.map(|_| {
					...
					thread::spawn(move || {
						loop {
							...
							let tensor = match deserialize_core_at(&core, &mut cp) {
								Some(t) => t,
								None => continue,
							};
							let chunk_sand = match Sandbag::from_bytes(&sand[ci.sand_pos..]) {
								Some(s) => s,
								None => continue,
							};

							let decompressed = if let Some(ref gt) = gt_cloned {
								if let Some(ref remap) = ci.remap {
									gt.decompress_with_remap(
										&chunk_sand,
										&tensor,
										Some(remap.clone()),
									)
								} else {
									tensor.decompress_all_global(&chunk_sand, gt)
								}
							} else {
								tensor.decompress_all(&chunk_sand)
							};
			...
			for (idx, ci) in chunks_data.iter().enumerate() {
				...
				let decompressed = if let Some(ref gt) = global.as_ref() {
					if let Some(ref remap) = ci.remap {
						gt.decompress_with_remap(&chunk_sand, &tensor, Some(remap.clone()))
					} else {
						tensor.decompress_all_global(&chunk_sand, gt)
					}
				} else {
		...
		log::debug!(
			"Decompressed {} in {:.2}s",
			name,
			start_decompress.elapsed().as_secs_f64()
		);
		if total_written != stats.element_count {
			log::error!(
				"decompressed {}/{} elements for {} ({} errors)",
				total_written,
				stats.element_count,
				name,
				worker_errors
