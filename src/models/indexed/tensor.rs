...
mod tests {
	...
	fn test_count_indexed_layout() {
		...
		println!(
			"  Sign bits: {} bytes, Round bits: {} bytes",
			packed.sign_bits.len(),
			packed.round_bits.len()
		);
	...
	fn test_large_tensor() {
		...
		println!(
			...
			packed.compressed_bytes(),
			packed.compressed_bytes() as f32 / 1024.0
		);
		println!(
			"  Original: {} bytes ({:.1} KB)",
			packed.original_bytes(),
			packed.original_bytes() as f32 / 1024.0
		);
		println!("  Ratio: {:.2}x", packed.comp_ratio());
		println!("  Max error: {:.6}", max_err);
