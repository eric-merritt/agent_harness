...
use crate::augment::augment::{
    Augment, AugmentBus, CompressedModelAugment,
    ModelTensorMap, TensorDescriptor, TensorDtype,
...
impl ModelServer {
    ...
    pub fn open_with_progress(model_dir: &Path, progress: Option<&crate::progress::LoadingProgress>) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let model_augment = CompressedModelAugment {
            ...
            manifest_path: model_dir.join("manifest.json").to_string_lossy().to_string(),
        ...
        log::info!("ModelServer: {} tensors, core ratio: {:.0}x",
            manifest.tensor_count, manifest.overall_core_ratio);

        // Initialize inference engine if config.json + tokenizer.json exist
        if let Some(p) = progress { p.set(5, "Loading inference engine..."); }
        ...
        let inference = if has_config && has_tokenizer {
            match InferenceEngine::open_with_progress(model_dir, progress) {
                Ok(mut engine) => {
                    log::info!("ModelServer: inference engine initialized, decompressing to INT4...");
                    engine.decompress_all_parallel(8, progress);
                    engine.finalize_mmap();
                    if let Some(p) = progress { p.set(100, "Model ready"); }
                    log::info!("ModelServer: all tensors decompressed and mmap finalized");
                    Some(Mutex::new(engine))
                }
                Err(e) => {
                    log::warn!("ModelServer: inference engine init failed: {} — running without generation", e);
                    None
                }
            }
        } else {
    ...
    pub fn tensor_names(&self) -> Vec<String> {
        let loader = self.loader.read().unwrap();
        loader.tensor_names().iter().map(|s| s.to_string()).collect()
    }
    ...
    pub fn print_summary(&self) {
        ...
        log::info!("  Original: {:.2} GB", self.manifest.total_original_bytes as f64 / 1e9);
        log::info!("  Core: {:.2} GB (ratio: {:.0}x)",
            self.manifest.total_core_bytes as f64 / 1e9,
            self.manifest.overall_core_ratio);
        log::info!("  Sandbag (disk): {:.2} GB", self.manifest.total_sandbag_bytes as f64 / 1e9);

        if let Some(ref plan) = plan {
            let gpu_count = plan.rules.iter().filter(|r| r.placement == TensorPlacement::Gpu).count();
            let cpu_count = plan.rules.iter().filter(|r| r.placement == TensorPlacement::Cpu).count();
        ...
        for t in self.manifest.tensors.iter().take(5) {
            log::info!("    {:<45} {:>10} elems  ratio: {:.0}x",
                t.name, t.element_count, t.core_ratio);
        }
    ...
    pub fn process_message(&self, msg: &str) -> String {
        log::info!("process_message: received \"{}\"", &msg[..msg.len().min(50)]);
        if let Some(ref inference) = self.inference {
            ...
            match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                ...
                Ok(output) if !output.is_empty() => {
                    log::info!("process_message: generated {} chars", output.chars().count());
                    output
                }
                Ok(_) => {
...
pub fn materialize_model(
    ...
    log::info!("Materialized {} tensors to {}", written, weights_path.display());
    Ok(written)
}
