...
impl GgufValueType {
    fn from_u32(v: u32) -> Option<Self> {
        match v {
            ...
            6 => Some(Self::F32), 7 => Some(Self::Bool), 8 => Some(Self::String),
...
impl<'a> Reader<'a> {
    ...
    fn read_u16(&mut self) -> Result<u16, String> {
        if self.pos + 2 > self.data.len() { return Err("EOF reading u16".into()); }
    ...
    fn read_u32(&mut self) -> Result<u32, String> {
        if self.pos + 4 > self.data.len() { return Err("EOF reading u32".into()); }
    ...
    fn read_u64(&mut self) -> Result<u64, String> {
        if self.pos + 8 > self.data.len() { return Err("EOF reading u64".into()); }
    ...
    fn read_f64(&mut self) -> Result<f64, String> {
        if self.pos + 8 > self.data.len() { return Err("EOF reading f64".into()); }
    ...
    fn read_string(&mut self) -> Result<String, String> {
        ...
        if self.pos + len > self.data.len() { return Err("EOF reading string".into()); }
...
impl GGUFFile {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        for _ in 0..tensor_count {
            ...
            tensor_info.push(GGUFTensorInfo { name, dim, dtype, offset });
        }
        ...
        Ok(Self { version, kv_meta, tensor_info, data_start, alignment })
    }
    ...
    pub fn dequantize_to_f32(
        ...
        match dtype {
            ...
            2 => {
                ...
                for b in 0..n_blocks {
                    ...
                    for i in 0..16 {
                        ...
                        if idx0 < element_count { out.push(d * q0); }
                        if idx1 < element_count { out.push(d * q1); }
                    }
                }
                out
            }
            13 => {
                ...
                for b in 0..n_blocks {
                    let off = b * block_size;
                    if off + block_size > raw.len() { break; }
...
impl ModelAdapter for GGUFFile {
    ...
    fn to_tensor_map(&self) -> crate::augment::augment::ModelTensorMap {
        let tensors: Vec<TensorDescriptor> = (0..self.tensor_count())
            ...
            .collect();
        let total = self.tensor_info.iter()
            .map(|t| t.byte_size())
            .sum();
...
impl ImportanceMatrix {
    ...
    pub fn from_bytes(data: &[u8]) -> Option<Self> {
        ...
        Some(Self { group_assignment, sign_bits, precision_lost, count })
    }
