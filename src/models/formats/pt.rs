...
impl ModelAdapter for PtFile {
    fn model_name(&self) -> &str {
        self.kv_meta.get("model_name").map(|s| s.as_str()).unwrap_or("pytorch")
    }

    fn tensor_count(&self) -> usize { self.tensor_info.len() }

    fn total_bytes(&self) -> u64 {
        self.tensor_info.values()
    ...
    fn raw_data(&self, tensor_name: &str) -> Option<&[u8]> {
        self.tensor_info.get(tensor_name).map(|info| {
            &self.tensor_raw[info.data_offsets[0]..info.data_offsets[1]]
        })
    }
}
...
pub enum PtDtype {
    Bool, Byte, Char, Short, Int, Long, Half, BFloat16, Float, Double,
}
