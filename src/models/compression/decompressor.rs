...
use crate::models::formats::sandbag::{GlobalTable, Sandbag};
...
impl DedupCountTensor {
	pub fn decompress_all(&self, sandbag: &Sandbag) -> Vec<f32> {
		...
		for (i, &(p_idx, t_idx)) in sandbag.index_signs.iter().enumerate() {
			let p_int = sandbag.unique_prefixes[p_idx as usize];
			let t_int = sandbag.unique_tails[t_idx as usize];
			let abs_w = (p_int as f32) / prefix_scale + (t_int) / tail_scale;
			let sign = (sandbag.index_signs[i / 8] >> (i % 8)) & 1;
			result[i] = if sign != 0 { -abs_w } else { abs_w };
		}

		// Restore outliers at full precision
		for &(pos, val) in &sandbag.meta_tensors {
			if pos < n {
				result[pos] = val;
			}
		}
	...
	pub fn decompress_all_global(&self, sandbag: &Sandbag, _global: &GlobalTable) -> Vec<f32> {
		...
		// Global table integration can be added later for cross-tensor compression
