...
pub unsafe fn quantize_block_avx512(weights: &[f32]) -> (f32, Vec<(usize, f32)>) {
	...
	let clip_idx = ((n as f64 * 0.995).ceil() as usize)
		.min(n)
		.saturating_sub(1);
	abs_vals.select_nth_unstable_by(clip_idx, |a, b| {
	...
	if is_x86_feature_detected!("avx512f") && n >= 16 {
		...
		let outliers_collected: Vec<(usize, f32)> = weights
			.par_iter()
			.enumerate()
			.filter_map(|(i, &w)| {
				if w.abs() > max_abs {
...
pub fn quantize_block_kl(weights: &[f32]) -> (f32, Vec<(usize, f32)>) {
	...
	let mut best_idx = ((n as f64 * 0.995).ceil() as usize)
		.min(n)
		.saturating_sub(1);
...
fn build_histogram(weights: &[f32], min_val: f32, max_val: f32, bins: usize) -> Vec<f32> {
	...
	if sum > 0.0 {
		hist.iter_mut().for_each(|x| *x /= sum);
	}
	hist
}
...
mod tests {
	...
	fn test_quantize_dequantize_roundtrip() {
		let weights = vec![
			0.001, -0.002, 0.0005, -0.0001, 0.5, -0.3, 0.0, 1e-6, 0.999, -0.999,
		];
