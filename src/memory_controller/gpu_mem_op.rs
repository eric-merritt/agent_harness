...
> {
	...
	// Prefer a discrete GPU. Device 0 is often the integrated adapter or the
	// llvmpipe software rasterizer, and silently benchmarking against a CPU
	// rasterizer is worse than failing.
		...
		.unwrap_or(phys_devices[0]);
	{
		...
		let name = props
			...
			.unwrap_or_default();
		eprintln!(
			"[GPU INIT] Found {} physical device(s), using {:?} ({})",
			phys_devices.len(),
			name,
	...
	eprintln!(
		"[GPU INIT] Compute queue family index: {}",
		queue_family_index
	);
