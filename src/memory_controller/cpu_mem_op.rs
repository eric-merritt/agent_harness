...
impl CpuMemory {
    pub fn new(total_size: usize) -> Self {
        let pool = MmapMut::map_anon(total_size)
            .expect("Failed to reserve CPU virtual memory space");
        Self { pool, total_size }
    }

    pub fn capacity(&self) -> usize { self.total_size }
    ...
    pub fn drop_page(&mut self, page_index: usize, page_size: usize) {
        ...
        if ret != 0 {
            log::warn!("madvise(MADV_DONTNEED) failed for page {}: {}", page_index, ret);
        }
    }
}
