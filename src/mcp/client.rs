...
impl McpClient {
	...
	pub async fn connect(&self, url: Option<&str>) -> Result<Vec<McpToolInfo>> {
		...
		if let Err(e) = self
			.jsonrpc_post(&target, "notifications/initialized", Value::Null)
			.await
		{
			log::warn!(
		...
		let list_result = self
			.jsonrpc_post(&target, "tools/list", Value::Null)
			.await?;
		...
		let tools: Vec<McpToolInfo> = {
			let arr = list_result
				...
				.and_then(|r| {
					r.get("tools")
						.cloned()
						.or_else(|| r.as_array().map(|a| Value::Array(a.clone())))
				})
		...
		log::info!(
			"MCP connection established to '{}' — {} tool(s) discovered",
			target,
			tools.len()
		);
		Ok(tools)
