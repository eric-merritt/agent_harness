...
impl Database {
	...
	pub async fn store_page_content(
		...
		log::info!(
			"Database: stored page content for url='{}', ref_id='{}'",
			url,
			ref_id
		);
	...
	pub async fn load_summary_ref(
		...
		let row = sqlx::query(r#"SELECT data FROM summary_refs WHERE ref_id = $1"#)
			.bind(ref_id)
			.fetch_optional(&*self.pool)
			.await?;
	...
	pub async fn get_cookies(&self, domain: &str) -> Result<Vec<(String, String)>, SqlxError> {
		let rows =
			sqlx::query(r#"SELECT name, value FROM cookies WHERE domain = $1 ORDER BY name"#)
				.bind(domain)
				.fetch_all(&*self.pool)
				.await?;
	...
	pub async fn get_all_cookies(&self) -> Result<Vec<(String, String, String)>, SqlxError> {
		let rows = sqlx::query(r#"SELECT domain, name, value FROM cookies ORDER BY domain, name"#)
			.fetch_all(&*self.pool)
			.await?;
	...
	pub async fn save_message(
		...
		let id = row.get::<Uuid, _>(0);
		log::info!(
			"Database: saved message id={} for conv={}",
			id,
			conversation_id
		);
		Ok(id)
	...
	pub async fn get_messages(
		...
		let count = rows.len();
		log::info!(
			"Database: fetched {} messages for conv={}",
			count,
			conversation_id
		);
