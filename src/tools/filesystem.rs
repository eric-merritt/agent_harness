...
impl Tool for ReadTool {
	...
	async fn call(&self, _ctx: &ToolContext, params: &Value) -> ToolResult {
		...
		let numbered: Vec<String> = subset
			.iter()
			.enumerate()
		...
		ToolResult::ok_with_data(
			format!("Read {} lines from {}", numbered.len(), path.display()),
			result,
		)
	}
}
...
impl Tool for InfoTool {
	...
	async fn call(&self, _ctx: &ToolContext, params: &Value) -> ToolResult {
		...
		let meta = match std::fs::metadata(&path) {
			...
			Err(e) => {
				log::warn!(
					"file_info: failed to get metadata for {}: {}",
					path.display(),
					e
				);
				return ToolResult::err(format!("{}: {}", e.kind(), e));
		...
		let content = std::fs::read_to_string(&path).unwrap_or_else(|e| {
			log::warn!(
				"file_info: could not read content of {}: {}",
				path.display(),
				e
			);
			String::new()
...
impl Tool for SummaryTool {
	...
	fn description(&self) -> &str {
		"Provide a structural summary of a file with line numbers for definitions."
	}
	...
	async fn call(&self, _ctx: &ToolContext, params: &Value) -> ToolResult {
		...
		for (i, line) in lines.iter().enumerate() {
			...
			if import_re.is_match(&trimmed_line) {
				structure
					.push(serde_json::json!({"type":"import","line":i+1,"content":&trimmed_line}));
		...
		let classes = structure
			.iter()
			.filter(|s| {
				s["kind"].as_str().map_or(false, |k| {
					k == "struct" || k == "enum" || k == "trait" || k == "union"
				})
			})
			.count();
...
impl Tool for WriteTool {
	...
	fn description(&self) -> &str {
		"Write content to a file. Defaults to append mode."
	}
	...
	async fn call(&self, _ctx: &ToolContext, params: &Value) -> ToolResult {
		...
		if let Some(parent) = path.parent() {
			if !parent.as_os_str().is_empty() {
				if let Err(e) = std::fs::create_dir_all(parent) {
					log::warn!(
						"write_file: failed to create parent dirs for {}: {}",
						path.display(),
						e
					);
				}
		...
		let size = std::fs::metadata(&path)
			...
			.unwrap_or_else(|e| {
				log::warn!(
					"write_file: could not read metadata for {}: {}",
					path.display(),
					e
				);
				0
...
impl Tool for GrepTool {
	...
	async fn call(&self, _ctx: &ToolContext, params: &Value) -> ToolResult {
		...
		let path = resolve(&path_str);
		let case_sensitive: bool = params
			.get("case_sensitive")
...
impl Tool for FindDefinitionTool {
	...
	fn description(&self) -> &str {
		"Find function and class definitions in a file."
	}
	...
	async fn call(&self, _ctx: &ToolContext, params: &Value) -> ToolResult {
		...
		if results.is_empty() {
			let target = name_filter
				.as_deref()
				.map(|n| format!("\"{}\"", n))
				.unwrap_or_else(|| "any definitions".to_string());
			return ToolResult::err(format!(
...
impl FindDefinitionTool {
	fn _find_def_end(&self, lines: &[&str], start_idx: usize) -> usize {
		let base_indent = lines[start_idx]
			.chars()
			.take_while(|source_character| source_character.is_whitespace())
			.count();
		let def_re =
			Regex::new(r"^(?:pub\s+)?(?:async\s+)?(?:fn|struct|enum|trait|union|impl)\s+\w+")
				.unwrap();
		let mut end_idx = start_idx;
		for i in (start_idx + 1)..lines.len() {
			let trimmed_source_line = lines[i].trim();
			if trimmed_source_line.is_empty() {
				continue;
			}
			let current_indent = lines[i]
				.chars()
...
impl Tool for ListDirTool {
	...
	fn description(&self) -> &str {
		"List files and directories in the provided directory."
	}
	...
	async fn call(&self, _ctx: &ToolContext, params: &Value) -> ToolResult {
		...
		if !src.is_dir() {
			...
			if src_ext != dst_ext && src_ext.as_ref().map(|e| !e.is_empty()).unwrap_or(false) {
				return ToolResult::err(
					"Changing file extension only allowed on previously empty extensions."
						.to_string(),
				);
			}
		}
		...
		if let Some(parent) = dst.parent() {
			if !parent.as_os_str().is_empty() {
				if let Err(e) = std::fs::create_dir_all(parent) {
					log::warn!(
						"move_file: failed to create parent dirs for {}: {}",
						dst.display(),
						e
					);
				}
	...
	async fn call(&self, _ctx: &ToolContext, params: &Value) -> ToolResult {
		...
		if let Some(ref tt) = target_type {
			match tt.as_str() {
				...
				_ => {
					return ToolResult::err(
						"Invalid target_type. Must be one of: [file,directory].".to_string(),
					);
				}
			}
		}
