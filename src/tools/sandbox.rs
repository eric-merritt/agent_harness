...
pub async fn run_with_timeout(
    ...
    log::debug!("Running tool '{}' with {}s timeout", tool.name(), timeout_secs.unwrap_or(DEFAULT_TIMEOUT_SECS));
    match tokio::time::timeout(timeout, tool.call(ctx, params)).await {
        ...
        Err(_) => {
            log::warn!("Tool '{}' exceeded {}s timeout", tool.name(), timeout_secs.unwrap_or(DEFAULT_TIMEOUT_SECS));
