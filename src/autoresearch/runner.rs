...
use super::report::{IterationResult, ResearchReport};
...
impl<M: Metric> AutoResearchRunner<M> {
	...
	pub async fn run(&mut self) -> ResearchReport {
		...
		log::info!(
			"Autoresearch: baseline established — {}={:.4}",
			self.metric.name(),
			baseline
		);
		...
		log::info!(
			...
			self.report.total_improvements,
			self.report.terminated
		);
		self.report.clone()
	}
	...
	async fn run_iteration(&self, iteration: u32) -> IterationDecision {
		...
		IterationDecision {
			...
			notes: if kept {
				...
				format!(
					"improved but below threshold ({:.2}% < {:.2}%)",
