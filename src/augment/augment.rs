...
impl SystemPrompt {
    ...
    pub fn render(&self) -> String {
        ...
        for block in &self.blocks {
            ...
            if !active { continue; }
            match block {
                PromptBlock::Rule(r) => { out.push_str(&format!("Rule: {}\n", r.content)); }
                PromptBlock::PersonalityTrait(p) => { out.push_str(&format!("Trait: {}\n", p.content)); }
                PromptBlock::Example(e) => { out.push_str(&format!("Input: {}\nOutput: {}\n", e.input, e.output)); }
                PromptBlock::ContextSnippet(c) => { out.push_str(&format!("Context ({}): {}\n", c.source, c.content)); }
            }
        }
        out
    }
...
pub enum AttachmentKind {
    Image { mime_type: String, width: Option<u32>, height: Option<u32> },
    Code { language: String, mime_type: Option<String> },
    Document { mime_type: String, page_count: Option<u32> },
    Audio { mime_type: String, duration_secs: Option<f64> },
    Video { mime_type: String, width: Option<u32>, height: Option<u32>, duration_secs: Option<f64> },
    Blob { mime_type: Option<String> },
}
...
pub enum TensorDtype {
    F32, F16, BF16, I8, I16, I32, I64, U8, Bool,
}
...
impl WeightPlacementPlan {
    ...
    pub fn plan(
        ...
        for t in &tensor_map.tensors {
            if t.layer_index.is_none() { continue; }
        ...
        for t in &tensor_map.tensors {
            if rules.iter().any(|r| r.tensor_name == t.name) { continue; }
        ...
        for edit in &self.edits {
            ...
            match &edit.method {
                ...
                AblationMethod::SvdSteering { u, scale } => {
                    ...
                    for u_row in u {
                        let proj: f32 = weights.iter().zip(u_row.iter())
                            .map(|(w, ui)| w * ui).sum();
...
impl AugmentBus {
    ...
    pub fn plug(&mut self, augment: Augment, priority: u8) -> Uuid {
        let id = Uuid::new_v4();
        log::info!("AugmentBus: plugging augment id={} priority={}", id, priority);
        self.slots.push(AugmentSlot { id, augment, is_plugged: true, priority });
        self.slots.sort_by_key(|s| s.priority);
        log::debug!("AugmentBus: slots re-sorted by priority — {} total slots", self.slots.len());
        id
    }
