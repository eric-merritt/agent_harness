...
impl Shape {
	...
	pub fn from_slice(dims: &[usize]) -> Self {
		Self {
			dims: dims.iter().map(|&d| Dim::Known(d)).collect(),
		}
	}
	...
	pub fn permute(&self, axis_order: &[usize]) -> Option<Shape> {
		if axis_order.len() != self.rank() {
			return None;
		}
		let mut seen = vec![false; self.rank()];
		for &ax in axis_order {
			if ax >= self.rank() || seen[ax] {
				return None;
			}
			seen[ax] = true;
		}
		let permuted: SmallVec<[Dim; 4]> = axis_order
			.iter()
