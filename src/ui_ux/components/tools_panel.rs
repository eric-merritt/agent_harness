...
impl ToolsPanel {
	...
	pub fn render(&self, frame: &mut Frame, area: Rect) {
		...
		for card in &self.cards {
			canvas_h += if card.expanded {
				let params_count =
					(card.required_params.len() + card.optional_params.len()).min(10);
	...
	fn render_card_into(
		...
		scroll_view.render_widget(Block::default().style(Style::default().bg(card_bg)), area);

		let title_line = Line::from(Span::styled(&card.name, Style::default().fg(TEXT_FG)));
		...
		if dy < area.y + area.height {
			if !card.required_params.is_empty() || !card.optional_params.is_empty() {
				scroll_view.render_widget(
					Paragraph::new(Line::from("  Params:"))
						.style(Style::default().fg(TEXT_FG).bg(card_bg)),
					Rect {
						x: inner_x,
						y: dy,
						width: inner_w,
				...
				for param in &card.required_params {
					...
					scroll_view.render_widget(
						Paragraph::new(Line::from(format!("    [req] {}", param)))
							.style(Style::default().fg(REQ_FG).bg(card_bg)),
						Rect {
							x: inner_x,
							y: dy,
							width: inner_w,
				...
				for param in &card.optional_params {
					...
					scroll_view.render_widget(
						Paragraph::new(Line::from(format!("    [opt] {}", param)))
							.style(Style::default().fg(DIM_FG).bg(card_bg)),
						Rect {
							x: inner_x,
							y: dy,
		...
		if dy < area.y + area.height {
			scroll_view.render_widget(
				Paragraph::new(Line::from("  ... (TBD: invoke / disable / configure)"))
					.style(Style::default().fg(Color::Rgb(100, 100, 120)).bg(card_bg)),
				Rect {
					x: inner_x,
					y: dy,
					width: inner_w,
