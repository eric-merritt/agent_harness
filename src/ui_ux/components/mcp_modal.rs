...
pub fn detect_transport(endpoint: &str) -> &'static str {
    ...
    if lower.starts_with("http://") || lower.starts_with("https://") || lower.starts_with("ws://") || lower.starts_with("wss://") {
        if lower.contains("sse") || lower.contains("/sse") { "SSE" }
        else if lower.contains("stream") || lower.contains("streamable") { "Streamable HTTP" }
        else { "HTTP" }
    } else if lower.starts_with("grpc://") || lower.starts_with("grpc-") { "gRPC" }
    else if !endpoint.is_empty() { "Stdio" }
    else { "—" }
}
...
impl McpModal {
    ...
    pub fn connect(&self) -> Option<(String, String, bool, String, String)> {
        ...
        if matches!(*state, ConfigState::Connecting) { return None; }
        drop(state);
        log::info!("MCP connection test initiated: server={}, endpoint={}", self.server_name, self.endpoint);
        Some((
    ...
    pub fn is_connected(&self) -> bool {
        matches!(*self.config_state.read().unwrap_or_else(|p| p.into_inner()), ConfigState::Connected)
    }
    ...
    pub fn render(&self, frame: &mut Frame) {
        ...
        frame.render_widget(
            Paragraph::new(Line::from(Span::styled(header_text, Style::default().fg(HEADER_FG).bg(header_bg)))),
        ...
        let render_field = |frame: &mut Frame, label: &str, value: &str, placeholder: &str, field: &FocusedField, focused: &FocusedField, y: &mut u16| {
            let lbl = Paragraph::new(Line::from(Span::styled(label, Style::default().fg(LABEL_FG))));
            frame.render_widget(lbl, Rect { x: inner.x, y: *y, width: label_w, height: 1 });
            let ia = Rect { x: inner.x + label_w + 1, y: *y, width: field_w, height: 1 };
            let text = if value.is_empty() && field != focused { placeholder } else { value };
        ...
        render_field(frame, "Name:", &self.server_name, "Enter server name...", &FocusedField::Name, &self.focused_field, &mut cy);
        render_field(frame, "Endpoint:", &self.endpoint, "URL, command, or path...", &FocusedField::Endpoint, &self.focused_field, &mut cy);

        cy -= 1;
        let detected = detect_transport(&self.endpoint);
        let det_text = if self.endpoint.is_empty() { "" } else { &format!("Type: {}", detected) };
        let det_color = if self.endpoint.is_empty() { STATUS_IDLE } else { LABEL_FG };
        frame.render_widget(
            Paragraph::new(Line::from(Span::styled(det_text, Style::default().fg(det_color))))
                .style(Style::default().bg(MODAL_BG)),
            Rect { x: inner.x, y: cy, width: inner.width, height: 1 },
        );
        cy += 2;

        let auth_x = inner.x;
        let auth_y = cy;
        let cb = if self.requires_auth { CHECKBOX_ON } else { CHECKBOX_OFF };
        let auth_line = Line::from(vec![
            Span::styled(format!("{} ", cb), Style::default().fg(if self.requires_auth { STATUS_OK } else { LABEL_FG })),
            Span::styled("Require Auth", Style::default().fg(LABEL_FG)),
        ]);
        self.auth_rect.replace(Some(Rect { x: auth_x, y: auth_y, width: 16, height: 1 }));
        frame.render_widget(Paragraph::new(auth_line).style(Style::default().bg(MODAL_BG)), Rect { x: auth_x, y: auth_y, width: field_w, height: 1 });
        cy += 2;

        if self.requires_auth {
            render_field(frame, "Username:", &self.username, "Enter username...", &FocusedField::Username, &self.focused_field, &mut cy);
            render_field(frame, "Password:", &self.password, "Enter password...", &FocusedField::Password, &self.focused_field, &mut cy);
        }
        ...
        let render_btn = |frame: &mut Frame, label: &str, bg: Color, area: Rect, disabled: bool| {
            ...
            let fg = if disabled { Color::Rgb(100, 100, 100) } else { BTN_FG };
            let pad = (btn_w as usize).saturating_sub(label.len()) / 2;
            let mut chars: Vec<Span> = Vec::with_capacity(btn_w as usize);
            for _ in 0..pad { chars.push(Span::styled(" ", Style::default().fg(fg).bg(bg))); }
            for c in label.chars() { chars.push(Span::styled(c.to_string(), Style::default().fg(fg).bg(bg))); }
            while chars.len() < btn_w as usize { chars.push(Span::styled(" ", Style::default().fg(fg).bg(bg))); }
            frame.render_widget(Paragraph::new(Line::from(chars)), area);
        };

        let save_area = Rect { x: btn_start_x, y: btn_y, width: btn_w, height: 1 };
        self.save_rect.replace(Some(save_area));
        render_btn(frame, "Save", BTN_SAVE_BG, save_area, is_busy || !is_valid);

        let cancel_area = Rect { x: btn_start_x + btn_w + gap, y: btn_y, width: btn_w, height: 1 };
    ...
    pub fn handle_key(&mut self, ke: &crossterm::event::KeyEvent) -> ModalAction {
        ...
        let is_busy = matches!(*state, ConfigState::Connecting);
        drop(state);
        if is_busy { return ModalAction::None; }
        if !self.is_valid() && ke.code == KeyCode::Enter { return ModalAction::None; }

        match ke.code {
            KeyCode::Esc => ModalAction::Close,
            KeyCode::Tab => {
                let fields = if self.requires_auth {
                    vec![FocusedField::Name, FocusedField::Endpoint, FocusedField::Username, FocusedField::Password]
                } else {
            ...
            KeyCode::Backspace => {
                match self.focused_field {
                    FocusedField::Name => { self.server_name.pop(); }
                    FocusedField::Endpoint => { self.endpoint.pop(); }
...
pub enum ModalAction { None, Save, Close, ToggleAuth }
