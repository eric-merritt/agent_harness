    ...
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        if has_text {
            ...
            let visible: Vec<String> = if all_wrapped.len() > MAX_VISIBLE_LINES {
                all_wrapped[all_wrapped.len() - MAX_VISIBLE_LINES..].iter().cloned().collect()
            } else {
                all_wrapped.clone()
            };
            ...
            let effective_cursor_line = cursor_line.min(visible_start.saturating_add(visible.len()).saturating_sub(1));
            ...
            for (i, line) in visible.iter().enumerate() {
                ...
                let is_cursor_line = effective_cursor_line >= visible_start && effective_cursor_line < visible_start + visible.len() && effective_cursor_line - visible_start == i;
                let cursor_col_in_this_line = if is_cursor_line {
                    // Recalculate column within this specific line
                    let chars_before: usize = all_wrapped[visible_start..abs_line_idx].iter().map(|l| l.chars().count()).sum();
                ...
                for ch in line.chars() {
                    ...
                    let is_at_cursor = is_cursor_line && cursor_col_in_this_line == (x - inner_left) as usize;
