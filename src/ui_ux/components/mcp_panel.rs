...
impl Default for McpPanel {
	fn default() -> Self {
		Self {
			...
			card_rects: Vec::new(),
			tool_tree: std::sync::Arc::new(std::sync::RwLock::new(None)),
			expanded_groups: std::collections::HashSet::new(),
			scroll_state: Arc::new(RwLock::new(ScrollViewState::new())),
			group_rects: std::cell::RefCell::new(Vec::new()),
		}
	}
}
...
impl McpPanel {
	...
	pub fn add_card(
		...
		if self
			.cards
			.iter()
			.any(|c| c.name == name && c.endpoint == endpoint)
		{
			return;
		}
	...
	pub fn render(&mut self, frame: &mut Frame, area: Rect) {
		...
		// ── Draw borders, header, and "+" button into the frame buffer ─
		{
			...
			let btn_y = bottom - 1;
			self.add_button_rect = Some(Rect {
				x: btn_x,
				y: btn_y,
				width: btn_len,
				height: 1,
		...
		for card in &self.cards {
			...
			if screen_y + card_h as i32 > content_area.y as i32
	...
	fn render_card_into(
		...
		scroll_view.render_widget(Block::default().style(Style::default().bg(row_bg)), area);

		let status_icon = if card.connected {
			"\u{25CF}"
		} else {
			"\u{25CB}"
		};
		let status_color = if card.connected {
			CONNECTED_FG
		} else {
			DISCONNECTED_FG
		};
		let prefix = if card.expanded {
			"\u{25BE} "
		} else {
			"\u{25B8} "
		};
		...
		let display_name: String = if card.name.chars().count() > max_name as usize {
			format!(
				...
				card.name
					.chars()
					.take((max_name as usize).saturating_sub(1))
					.collect::<String>()
			)
		} else {
			card.name.clone()
		...
		match *tree {
			Some(ref nodes) if !nodes.is_empty() => {
				for node in nodes {
					...
					self.group_rects.borrow_mut().push((
						node.name.clone(),
						Rect {
							x: inner_x,
							y: dy,
					...
					if is_expanded {
						for child in &node.children {
							...
							let child_icon = if child.is_leaf {
								"\u{25B8}"
							} else {
								"\u{25BE}"
							};
							...
							buf.set_string(
								...
								dy,
								&child_label,
								Style::default().fg(DIM_FG).bg(row_bg),
							);
							dy += 1;
						}
					}
