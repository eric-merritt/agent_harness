...
impl MessagesPanel {
	...
	fn begin_scrollbar_drag(&mut self, mouse_row: u16, area: Rect) {
		...
		let new_offset =
			(rel_row as f64 / track_height.max(1) as f64 * scrollable_rows as f64).round() as u16;
		...
		self.scrollbar_drag = Some(ScrollbarDrag { scrollable_rows });
	}
	...
	fn continue_scrollbar_drag(&mut self, mouse_row: u16, area: Rect) {
		...
		let new_offset = (rel_row as f64 / track_height.max(1) as f64 * drag.scrollable_rows as f64)
			.round() as u16;
	...
	pub fn render(&self, frame: &mut Frame, area: Rect) {
		...
		for (i, msg) in messages.iter().enumerate() {
			...
			let columns = Layout::default()
				...
				.constraints([Constraint::Length(this_prefix_width), Constraint::Min(1)])
				.split(msg_area);
		...
		*state = scroll_state;
		...
		if track_height > 0 && scrollable > 0 {
			...
			let thumb_height = ((track_height as f64 * content_area.height as f64)
				/ canvas_h as f64)
				.max(1.0) as u16;
			let thumb_top = (current_offset.y as f64 / max_offset
