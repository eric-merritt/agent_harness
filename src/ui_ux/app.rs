...
impl App {
	...
	fn start_connection_test(&self) {
		let Some((name, endpoint, requires_auth, username, password)) = self.mcp_modal.connect()
		else {
			return;
		};
		...
		*state.write().unwrap_or_else(|p| p.into_inner()) = ConfigState::Connecting;
		...
		tokio::spawn(async move {
			...
			let result: Result<String, String> = async {
				...
				let mut req = client.get(raw).timeout(std::time::Duration::from_secs(5));
				if requires_auth && !username.is_empty() {
					req = req.basic_auth(&username, Some(&password));
				}
				if let Ok(resp) = req.send().await {
				...
				if !cmd.is_empty()
					&& (std::process::Command::new("which")
						.arg(cmd)
						.output()
						.is_ok() || std::path::Path::new(cmd).exists())
				{
			...
			*state = match result {
				Ok(transport) => {
					*pending.write().unwrap_or_else(|p| p.into_inner()) =
						Some((name, transport, endpoint.clone(), 0));
	...
	pub fn run(mut self) {
		...
		if let Err(e) = crossterm::execute!(stdout, EnableMouseCapture, EnableBracketedPaste,) {
		...
		if self.show_mcp_modal && self.mcp_modal.is_connected() {
			if let Ok(mut pending) = self.pending_mcp_server.write() {
				if let Some((name, transport, endpoint, tools)) = pending.take() {
					...
					config.add_server(SavedMcpServer {
						name,
						transport,
						endpoint,
					});
				...
				match ke.kind {
					KeyEventKind::Press | KeyEventKind::Repeat => {
						...
						if scroll_allowed && self.chat.scroll_messages(&ke) {
							...
							let mut state =
								input_bar.state.write().unwrap_or_else(|p| p.into_inner());
					...
					if !text.is_empty() {
						...
						if let Some((conv_id, content)) = chat_ref.submit_sync(text) {
							...
							if let Some(db_pool) = chat_ref.db.clone() {
								tokio::spawn(async move {
									if let Err(e) =
										db_pool.save_message(conv_id, "user", &content).await
									{
										log::error!("DB save failed: {}", e);
									}
								});
