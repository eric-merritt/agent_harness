...
impl<'a> ChatInterface<'a> {
    pub fn new(db: Option<Database>) -> Self {
        ...
        Self {
            ...
            state: Arc::new(RwLock::new(ChatState {
                ...
                submit_button: SubmitButton { label: "Send", is_pressed: false, is_hovered: false },
    ...
    pub fn submit_button(&self) -> SubmitButton<'static> {
        SubmitButton { label: "Send", is_pressed: false, is_hovered: false }
    }
    ...
    pub fn deliver_response(&self, response: String) {
        log::info!("model response received ({} chars)", response.chars().count());
        let mut state = self.state.write().unwrap_or_else(|p| p.into_inner());
        // Find the last assistant message and update it
        if let Some(last) = state.conversation.messages.iter_mut().rev().find(|m| m.role == Role::Agent) {
