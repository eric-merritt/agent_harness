...
impl ChatApiClient {
    pub fn new(endpoint: &str, model: &str) -> Self {
        let client = reqwest::Client::builder()
            ...
            .build()
            .unwrap_or_else(|e| {
                log::error!("failed to build reqwest client with custom timeout, using default: {}", e);
                reqwest::Client::new()
            });
        log::debug!("chat client created: endpoint={}, model={}", endpoint, model);
        Self {
        ...
        G: Fn(Option<anyhow::Error>) + Send + Sync,
    {
        ...
        let status = resp.status();
        if !status.is_success() {
            log::warn!("stream chat completions returned HTTP {} from {}", status, url);
        ...
        log::debug!("stream completed ({} chars total)", full_response.chars().count());
        on_done(None);
        Ok(full_response)
    }
}
