...
impl<'a> FfnBlock<'a> {
    ...
    pub fn from_quantized_refs(
        gate_packed: &'a [u8], gate_scales: &'a [f32],
        up_packed: &'a [u8], up_scales: &'a [f32],
        down_packed: &'a [u8], down_scales: &'a [f32],
    ) -> Self {
        Self {
            gate_w: &[], up_w: &[], down_w: &[],
            gate_packed, gate_scales,
    ...
    pub fn forward(&self, input: &[f32], state: &mut FfnState, config: &ModelConfig) -> Vec<f32> {
        ...
        if is_quantized {
            ...
            swiglu_4bit_into(
                &mut out,
                input,
                self.gate_scales, self.gate_packed,
                self.up_scales, self.up_packed,
                self.down_scales, self.down_packed,
                n_embd, n_ff, group_size,
...
pub fn swiglu_4bit_into(
    out: &mut [f32],
    input: &[f32],
    gate_scales: &[f32], gate_packed: &[u8],
    up_scales: &[f32], up_packed: &[u8],
    down_scales: &[f32], down_packed: &[u8],
    n_embd: usize,
    n_ff: usize,
    group_size: usize,
    scratch_gate: &mut [f32],
    ...
    math::gemv_4bit_into(scratch_gate, gate_scales, gate_packed, input, n_ff, n_embd, group_size);
    math::gemv_4bit_into(scratch_up, up_scales, up_packed, input, n_ff, n_embd, group_size);
    for i in 0..n_ff {
        scratch_act[i] = math::silu(scratch_gate[i]) * scratch_up[i];
    }
    math::gemv_4bit_into(out, down_scales, down_packed, scratch_act, n_embd, n_ff, group_size);
}
