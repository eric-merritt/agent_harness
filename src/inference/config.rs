...
impl ModelConfig {
    pub fn from_gguf(gguf: &GGUFFile) -> Self {
        ...
        let get_u64 = |key: &str| -> u64 {
            gguf.kv_meta.get(key)
                .and_then(|v| match v { GGUFValue::U64(v) => Some(*v), GGUFValue::U32(v) => Some(*v as u64), _ => None })
                .unwrap_or(0)
        };
        let get_f32 = |key: &str| -> f32 {
            gguf.kv_meta.get(key)
                .and_then(|v| match v { GGUFValue::F32(v) => Some(*v), _ => None })
                .unwrap_or(0.0)
        };

        // Detect architecture prefix from GGUF metadata
        let prefix = gguf.kv_meta.get("general.architecture")
        ...
        let vocab_size = gguf.tensor_info.iter()
            .find(|t| t.name == "token_embd.weight")
            .map(|t| t.dim[1] as usize)
            .unwrap_or(0);

        let rope_sections: [i32; 4] = gguf.kv_meta
            .get(&format!("{}rope.dimension_sections", prefix))
            .and_then(|v| match v {
                GGUFValue::Array(arr) => {
                    let vals: Vec<i32> = arr.data.iter()
        ...
        let is_recurrent = (0..n_layer_all)
            .map(|i| {
                if i >= n_layer { return false; }
                if i == n_layer - 1 { return false; }
                (i + 1) % full_attn_interval.max(1) != 0
            })
            .collect();

        // Safe resolution baseline for GGUF contexts
        let max_seq_len = if context_length > 0 { context_length } else { 4096 };
        
        Self {
            n_layer, n_layer_nextn, n_embd, n_ff,
            n_head, n_head_kv, n_embd_head,
            rope_dim_count, rope_sections, rope_freq_base, rms_eps,
            ssm_d_conv, ssm_d_inner, ssm_d_state, ssm_dt_rank, ssm_n_group,
            full_attn_interval, vocab_size, context_length,
            eos_token_id, pad_token_id, is_recurrent,
            layer_types: Vec::new(),
            max_seq_len,
        }
    }
    ...
    pub fn from_dir(dir: &std::path::Path) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        if config.max_seq_len == 0 {
            config.max_seq_len = if config.context_length > 0 { config.context_length } else { 4096 };
        }
            ...
            config.is_recurrent = (0..n_layer_all)
                .map(|i| {
                    ...
                    if let Some(l_type) = config.layer_types.get(i) {
                        ...
                        if i == config.n_layer - 1 { return false; }
