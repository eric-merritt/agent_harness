...
impl<'a> AttnBlock<'a> {
    ...
    pub fn from_refs(
        ...
        Self { wq, wk, wv, wo, q_norm, k_norm }
    }
    ...
    pub fn forward(
        ...
        assert!(pos < kv.max_seq_len, "pos {} exceeds max_seq_len {}", pos, kv.max_seq_len);
        assert_eq!(input.len(), n_embd, "input len {} != n_embd {}", input.len(), n_embd);
        ...
        if has_gate {
            math::gemv_into(&mut state.q_gate_fused, self.wq, input, n_embd_head * 2 * n_head, n_embd);
            let q_stride = n_embd_head * n_head;
            state.q.copy_from_slice(&state.q_gate_fused[0..q_stride]);
            state.gate.copy_from_slice(&state.q_gate_fused[q_stride..2 * q_stride]);
        } else {
        ...
        if has_q_norm {
            for h in 0..n_head {
                ...
                math::rms_norm_into(&mut state.q[start..end], &state.head_scratch, self.q_norm, rms_eps);
            }
        }

        // K and V projections 
        math::gemv_into(&mut state.k_cur, self.wk, input, n_embd_head * n_head_kv, n_embd);
        math::gemv_into(&mut state.v_cur, self.wv, input, n_embd_head * n_head_kv, n_embd);
        ...
        if has_k_norm {
            for h in 0..n_head_kv {
                ...
                math::rms_norm_into(&mut state.k_cur[start..end], &state.head_scratch, self.k_norm, rms_eps);
            }
        }
        ...
        for h in 0..n_head {
            ...
            math::rope_multi(
                ...
                pos, rope_dim_count, rope_sections, rope_freq_base,
            );
        }
        for h in 0..n_head_kv {
            let start = h * n_embd_head;
        ...
        for h in 0..n_head {
            ...
            for s in active_scores.iter_mut() { *s = 0.0; }
            ...
            for s in active_scores.iter_mut() { *s *= scale; }
        ...
        math::gemv_into(&mut out, self.wo, &state.proj_input[0..attn_total_dim], n_embd, attn_total_dim);
        out
    }
}
...
mod tests {
    ...
    fn test_forward_with_state() {
        ...
        for v in kv.k.iter_mut() { *v = 0.0; }
        for v in kv.v.iter_mut() { *v = 0.0; }
