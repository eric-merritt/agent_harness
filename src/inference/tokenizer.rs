...
impl Tokenizer {
        ...
        for (rank, merge) in merges.iter().enumerate() {
            if let Some(space_idx) = merge.find(' ') {
                ...
                if let (Some(&id1), Some(&id2)) =
                    (token_to_id.get(first), token_to_id.get(second))
                {
    ...
    pub fn extract_data(gguf: &GGUFFile) -> TokenizerData {
        let get_string = |key: &str| -> String {
            gguf.kv_meta.get(key)
                .and_then(|v| match v { GGUFValue::String(s) => Some(s.clone()), _ => None })
                .unwrap_or_default()
        };
        let get_u32 = |key: &str| -> u32 {
            gguf.kv_meta.get(key)
                .and_then(|v| match v { GGUFValue::U32(v) => Some(*v), GGUFValue::U64(v) => Some(*v as u32), _ => None })
                .unwrap_or(0)
        };
        let get_string_array = |key: &str| -> Vec<String> {
            gguf.kv_meta.get(key)
                .and_then(|v| match v {
                    GGUFValue::Array(arr) => Some(arr.data.iter().filter_map(|item| {
                        if let GGUFValue::String(s) = item { Some(s.clone()) } else { None }
                    }).collect()),
                    _ => None,
                }).unwrap_or_default()
        };
        let get_u32_array = |key: &str| -> Vec<u32> {
            gguf.kv_meta.get(key)
                .and_then(|v| match v {
                    GGUFValue::Array(arr) => Some(arr.data.iter().filter_map(|item| match item {
                        GGUFValue::U32(v) => Some(*v), GGUFValue::U64(v) => Some(*v as u32), _ => None,
                    }).collect()),
                    _ => None,
                }).unwrap_or_default()
        };
    ...
    fn from_data(data: TokenizerData) -> Self {
        let (vocab, merges) = if data.tokens.is_empty() {
            let model = data.model.as_object().map(|m| m.clone()).unwrap_or_default();
        ...
        Self { vocab, token_type, eos_token_id, pad_token_id, merge_ranks, token_to_id, byte_to_unicode, unicode_to_byte }
    }
    ...
    pub fn from_dir(dir: &std::path::Path) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        if cache_path.exists() {
            ...
            match bincode::deserialize::<Tokenizer>(&data) {
                Ok(tok) => {
                    log::info!("Tokenizer: loaded from binary cache ({} tokens)", tok.vocab.len());
                    return Ok(tok);
                }
                Err(e) => {
                    log::warn!("Tokenizer: binary cache corrupt ({}), falling back to JSON", e);
                }
            }
        }
    ...
    pub fn extract_to_file(gguf: &GGUFFile, dir: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn bpe(&self, chars: &[char]) -> Vec<u32> {
        ...
        loop {
            ...
            let merged_str = format!(
                "{}{}",
                self.vocab.get(id1 as usize).map(|s| s.as_str()).unwrap_or(""),
                self.vocab.get(id2 as usize).map(|s| s.as_str()).unwrap_or(""),
            );
