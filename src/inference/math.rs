...
pub fn gemv_into(out: &mut [f32], w: &[f32], x: &[f32], rows: usize, cols: usize) {
    ...
    let nthreads = std::thread::available_parallelism().map(|n| n.get()).unwrap_or(4).min(32).max(1);
...
pub fn quantize(weights: &[f32], group_size: usize) -> (Vec<f32>, Vec<u8>) {
    ...
    for g in 0..n_groups {
        ...
        for (i, &w) in group.iter().enumerate() {
            let q = if scale > 0.0 { (w * inv_scale).round() as i32 } else { 0 };
...
fn gemv_4bit_worker_dispatch(
    ...
    if is_x86_feature_detected!("avx512f") && is_x86_feature_detected!("avx512bw") && group_size == 32 && cols % 128 == 0 {
        unsafe {
            gemv_4bit_parallel_worker_avx512(out, global_scales, global_packed, x, row_start, row_end, cols);
        }
    } else {
        gemv_4bit_parallel_worker_scalar(out, global_scales, global_packed, x, row_start, row_end, cols, group_size);
    }
}
...
pub unsafe fn gemv_4bit_parallel_worker_avx512(
    ...
    for i in row_start..row_end {
        ...
        for c in 0..chunks_128 {
            ...
            let w0_1 = _mm512_fmadd_ps(_mm512_add_ps(_mm512_cvtepi32_ps(_mm512_cvtepu8_epi32(_mm512_castsi512_si128(lin_0_63))), v_offset), scale_0, _mm512_setzero_ps());
            let w0_2 = _mm512_fmadd_ps(_mm512_add_ps(_mm512_cvtepi32_ps(_mm512_cvtepu8_epi32(_mm512_extracti32x4_epi32(lin_0_63, 1))), v_offset), scale_0, _mm512_setzero_ps());
            ...
            let w1_1 = _mm512_fmadd_ps(_mm512_add_ps(_mm512_cvtepi32_ps(_mm512_cvtepu8_epi32(_mm512_extracti32x4_epi32(lin_0_63, 2))), v_offset), scale_1, _mm512_setzero_ps());
            let w1_2 = _mm512_fmadd_ps(_mm512_add_ps(_mm512_cvtepi32_ps(_mm512_cvtepu8_epi32(_mm512_extracti32x4_epi32(lin_0_63, 3))), v_offset), scale_1, _mm512_setzero_ps());
            ...
            let w2_1 = _mm512_fmadd_ps(_mm512_add_ps(_mm512_cvtepi32_ps(_mm512_cvtepu8_epi32(_mm512_castsi512_si128(lin_64_127))), v_offset), scale_2, _mm512_setzero_ps());
            let w2_2 = _mm512_fmadd_ps(_mm512_add_ps(_mm512_cvtepi32_ps(_mm512_cvtepu8_epi32(_mm512_extracti32x4_epi32(lin_64_127, 1))), v_offset), scale_2, _mm512_setzero_ps());
            ...
            let w3_1 = _mm512_fmadd_ps(_mm512_add_ps(_mm512_cvtepi32_ps(_mm512_cvtepu8_epi32(_mm512_extracti32x4_epi32(lin_64_127, 2))), v_offset), scale_3, _mm512_setzero_ps());
            let w3_2 = _mm512_fmadd_ps(_mm512_add_ps(_mm512_cvtepi32_ps(_mm512_cvtepu8_epi32(_mm512_extracti32x4_epi32(lin_64_127, 3))), v_offset), scale_3, _mm512_setzero_ps());
