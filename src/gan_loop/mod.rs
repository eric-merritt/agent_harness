...
pub use state::{GANLoopState, TaskItem, TaskStatus, GANLoopConfig, GANTerminationReason};
...
pub async fn run_loop(
    mut loop_state: GANLoopState,
    config: GANLoopConfig,
) -> GANLoopState {
    use tokio::time::{timeout, Duration};

    log::info!("GAN loop starting — goal: '{}', max_iterations: {}", loop_state.goal, config.max_iterations);
    loop_state.start();
    ...
    let _ = timeout(loop_timeout, async {
        ...
        log::info!("GAN loop planning complete — {} tasks", loop_state.tasks.len());
