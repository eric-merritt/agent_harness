...
const GENERATOR_SYSTEM: &str =
    "You are a generating agent. Your job is to produce a concrete, complete \
    ...
    for task in &mut state.tasks {
        ...
        match generation_text {
            Some(content) => {
                ...
                log::info!("[Generator] Task '{}' — iteration {} complete", task.description, task.iteration_count);
            }
            None => {
                ...
                task.evaluator_notes = None;
                task.iteration_count += 1;
                log::warn!("[Generator] Task '{}' — LLM call failed, using placeholder", task.description);
            }
        }
    }
}
