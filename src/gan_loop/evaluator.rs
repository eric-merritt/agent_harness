...
const EVALUATOR_SYSTEM: &str = "You are an evaluating agent. Review the generation for the given task. \
		...
		match verdict {
			...
			None => {
				...
				task.evaluator_notes =
					Some("[Evaluator LLM unavailable — marked complete by default]".to_string());
				log::warn!(
					"[Evaluator] Task '{}' — LLM unavailable, defaulting to complete",
					task.description
				);
			}
		}
