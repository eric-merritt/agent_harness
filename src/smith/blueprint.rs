...
impl Blueprint {
    ...
    pub fn validate(&self) -> Vec<String> {
        ...
        let slot_ids: std::collections::HashSet<_> = self.slots.iter().map(|s| s.id.as_str()).collect();
        for link in &self.links {
            if !slot_ids.contains(link.from.as_str()) {
                errors.push(format!("Link '{}' references missing slot '{}'", link.label, link.from));
            }
            if !slot_ids.contains(link.to.as_str()) {
                errors.push(format!("Link '{}' references missing slot '{}'", link.label, link.to));
            }
        }
        ...
        for slot in &self.slots {
            if !visited.contains(&slot.id) {
                Self::dfs_validate(&self.slots, &self.links, &slot.id, &mut visited, &mut rec_stack, &mut errors);
            }
        }
        ...
        for link in links {
            if link.from == node {
                if rec_stack.contains(&link.to) {
                    errors.push(format!("Circular dependency detected: {} → {}", link.from, link.to));
